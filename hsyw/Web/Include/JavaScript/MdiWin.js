...
function mywin()
{
	...
	function activewin(oEl)                 //激活窗口
	{
		...
		this.currentwin.wintitle.style.backgroundImage = 'url(Images/Main/tab1.gif)';
		oEl.wintitle.style.zIndex = tempzindex;
		oEl.style.display = "";
		oEl.wintitle.style.backgroundImage = 'url(Images/Main/tab2.gif)';
		this.currentwin = oEl;
		...
		framewin = this.currentwin.document.frames[oEl.index-1];
		//framewin.location = framewin.location;
	...
	function refreshwin(obj)  //刷新页面
	{
	   ...
	   // this.removewin(obj);
	    if (index > -1) {
	        framewin = obj.document.frames[index];
	        framewin.location = framewin.location;  //+ "&".concat(Math.random());
	    }
	...
	function addwin(url,title)                                        //方法的具体实现
	{
		...
		oTitle.style.backgroundImage = 'url(Images/Main/tab2.gif)';
		...
		oTitle.ondblclick = new Function("win.removewin(this.win)");
