...
//------------------ 样式定义 ---------------------------//
//功能按钮同样样式
var s_tiannet_turn_base = "height:16px;font-size:9pt;color:white;border:0 solid #CCCCCC;cursor:hand;background-color:#94CAF7;";
//翻年、月等的按钮
...
//日期选择控件体的样式
var s_tiannet_body = "width:160px;height:170px;background-color:#94CAF7;display:none;z-index:9998;position:absolute;" +
...
//字体样式
var s_tiannet_font = "color:white;font-size:9pt;cursor:hand;";
//链接的样式
...
var tiannetMinute = tiannetDateNow.getMinutes(); //tiannetDateNow.getMinutes();
...
var tiannetYearEnd = tiannetYear; //可选择的结束年份
var tiannetYearSt = tiannetYearEnd-10; //可选择的开始年份
...
document.write('<div name="divTiannetDate" id="divTiannetDate" style="' + s_tiannet_body + '" style="本日历选择控件由tiannet根据前人经验完善而成！">');
document.write('<div align="center" id="divTiannetDateText" Author="tiannet" style="padding-top:2px;">');
document.write('<table border=0 cellspacing=0 cellpadding=0 style="width:149px;"><tr style="height:22px;"><td style="width:18px;" align="center">');
//上一月
document.write('<input type="button" style="' + s_tiannet_turn + '" value="<<" title="上一月" onClick="tiannetPrevMonth();">');
document.write('</td><td style="width:1px;"></td><td style="width:60px;" align="center">');
//年
...
document.write('</select>');
document.write('</td><td style="width:1px;"></td><td style="width:18px;" align="center">');
//下一月
...
//输出一条横线
//document.write('<div style="' + s_tiannet_line + '"></div>');
document.write('<table border=0 cellspacing=0 cellpadding=0 bgcolor=white onselectstart="return false" style="width:160px;">');
document.write(' <tr style="background-color:#94CAF7;font-size:10pt;color:white;height:22px;" Author="tiannet">');
...
//输出天的选择
document.write('<table border=0 cellspacing=1 cellpadding=0 bgcolor="#F0FFF0" onselectstart="return false" style="width:160px;">');
var n = 0;
for (var i = 0; i < 5; i++) {
...
document.write('<td align="right" colspan="5"><a href="javascript:tiannetClear();" style="' + s_tiannet_link + '">清空</a>' +
       ...
       '</td>');
document.write(' </tr>');
document.write('<tr id = "TR_HM" style="background-color:#94CAF7; height:23px;"><td colspan="7" style="width:149px;"><table align="center" border=0 cellspacing=0 cellpadding=0 style="height:100%;width:100%"><td align="center" style="width:34%;color:white">时间：</td><td align="center" style="width:33%">');
...
document.write('</select>');
document.write('</td><td align="center" style="width:33%;">');
...
function showTime() {
    ...
    hideElementsById(new Array("TR_HM","tiannetHourHead", "selTianHour", "tiannetMinuteHead", "selTianMinute"), true);
    if (m_bolShowHour) {
        //显示小时
        hideElementsById(new Array("TR_HM", "tiannetHourHead"), false);
        document.all.divTiannetDate.style.height = "188px";
    }
    if (m_bolShowMinute) {
        //显示分钟
        hideElementsById(new Array("TR_HM", "tiannetMinuteHead"), false);
        document.all.divTiannetDate.style.height = "188px";
    }
}
//弹出显示日历选择控件，以让用户选择
function tiannetPopCalendar(obj) {
    document.all.divTiannetDate.style.height = "170px";
    //隐藏下拉框，显示相对应的head
    hideElementsById(new Array("selTianYear", "selTianMonth", "selTianHour", "selTianMinute"), true);
    hideElementsById(new Array("TR_HM","tiannetYearHead", "tiannetMonthHead", "tiannetHourHead", "tiannetMinuteHead"), false);
    tiannetSetDay(tiannetYear, tiannetMonth);
    tiannetWriteHead();
    showTime();
    ...
    var h = document.all.divTiannetDate.style.pixelHeight;
    var w = document.all.divTiannetDate.style.pixelWidth;
    var x = obj.getBoundingClientRect().left;
    //计算left
    if (document.body.offsetWidth-x >w)
        iX =x;
    else
        iX = x - (w - obj.style.pixelWidth);
    if (iX < 0)
        iX = 0;
    //计算top

    if (obj.getBoundingClientRect().bottom + document.body.scrollTop + h > document.body.offsetHeight) {
        iY = obj.getBoundingClientRect().bottom + document.body.scrollTop - obj.style.pixelHeight - h-8;  
    }
    else {
        
        iY = obj.getBoundingClientRect().bottom + document.body.scrollTop;
    }
//    iY = window.event.y;
...
//显示日历控件(同时隐藏会遮挡的标签)
function tiannetShowControl(obj) {
