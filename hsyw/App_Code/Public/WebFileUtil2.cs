...
public class WebFileUtil2
{
        ...
        {
            string sql = string.Format("select t.* from T_FAU_FILELIST2 t where t.FILESORT='{0}'", FileSort);
            if (IsUseFull != "ALL")
            {
        ...
        {
            string sql = string.Format("select sum(FILESIZE) from T_FAU_FILELIST2");
        ...
        public static bool FileUpLoad(HtmlInputFile InputFile,string FileSort,string UserName)
        {
                ...
                if (GetAllFileSize() < maxSize)
                {
                    ...
                    string sql = string.Format("insert into T_FAU_FILELIST2 (FILEGUID,FILENAME,FILEURL,FILESORT,UPLOADNAME,UPLOADTIME,FILESIZE,ISUSEFULL) values('{0}','{1}','{2}','{3}','{4}',to_date('"+DateTime.Now.ToString()+"','YYYY-MM-dd HH24:MI:SS'),'{5}','1')",
        ...
        public static bool FileUpLoad(HtmlInputFile InputFile, string FileSort, string UserName,string OrgName)
        {
            ...
            double maxSize = Convert.ToDouble(cdata.GetZDZ("FJSC", "MAXSIZE")) * 1024 * 1024;
                ...
                if (GetAllFileSize() < maxSize)
                {
                    ...
                    string str_url = cdata.GetZDZ("FJSC", "URL") + "/" + FileSort + "/" + DateTime.Now.ToShortDateString() + "/" + fileNmae;
                    InputFile.PostedFile.SaveAs(str_path + fileNmae);
                    string sql = string.Format("insert into T_FAU_FILELIST2 (FILEGUID,FILENAME,FILEURL,FILESORT,UPLOADNAME,FILESIZE,ISUSEFULL,UPLOADORG) values('{0}','{1}','{2}','{3}','{4}',to_date('" + DateTime.Now.ToString() + "','YYYY-MM-dd HH24:MI:SS'),'{5}','1','{6}')",
        ...
        {
            string sql = string.Format("delete from T_FAU_FILELIST2 where FILEGUID = '{0}'", fileguid);
            DataFunction.ExecuteNonQuery(sql);
        }
